/*
NAME
  pool_test.go - a test suite adopting the golang testing library to test functionality of the
...
LICENSE
  pool_test.go is Copyright (C) 2020 the Australian Ocean Lab (AusOcean)
...
func TestRoundTrip(t *testing.T) {
	...
	for _, test := range roundTripTests {
		b := NewBuffer(test.len, test.max, test.timeout)
...
func BenchmarkRoundTrip(b *testing.B) {
	...
	// amortised cost.
	rb := NewBuffer(len, len*max, timeout)
...
func BenchmarkRoundTripWriterTo(b *testing.B) {
	...
	// of the ring buffer.
	buf := bytes.NewBuffer(make([]byte, 0, len*max+1))
